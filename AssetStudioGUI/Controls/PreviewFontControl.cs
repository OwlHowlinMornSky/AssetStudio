...
namespace AssetStudioGUI.Controls {
	public partial class PreviewFontControl : UserControl, IPreviewControl {
		...
		public void ResetPreview() {
			...
			_ = RemoveFontMemResourceEx(m_hfont);
			return;
		}
		...
		[DllImport("gdi32.dll")]
		private static extern Int32 RemoveFontMemResourceEx(IntPtr hFont);
		...
		internal void PreviewFont(Font m_Font) {
			if (m_Font.m_FontData != null) {
				...
				uint cFonts = 0;
				m_hfont = AddFontMemResourceEx(data, (uint)m_Font.m_FontData.Length, IntPtr.Zero, ref cFonts);
				if (m_hfont == IntPtr.Zero) {
					return;
				}
